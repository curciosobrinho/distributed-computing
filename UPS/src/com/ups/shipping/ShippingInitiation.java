...
public class ShippingInitiation {
	...
	public JsonObject initiateShipping(JsonObject incomingJson) {
		...
		System.out.println("UPS\n===\n" + organization + " requested shipping of " + numberOfItemsToShip
				+ " items to ZIP: " + zip + ", Order Ref: " + orderRefId);

		// Persist the shipping request and generate a shipping reference
		// skip for now, create a hard-coded shipping number - PK

		JsonObject responseJson = Json.createObjectBuilder().add("Accepted", true).add("ShippingReferenceNumber", (1000+orderRefId))
				.add("Organization", organization).add("OrderRefId", orderRefId).build();
